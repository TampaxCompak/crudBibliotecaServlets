...
public class DAOGenerico<T, ID> {
    ...
    public List<Prestamo> getPrestamosActivosPorUsuario(int usuarioId) {
        return em.createQuery("SELECT p FROM Prestamo p WHERE p.usuario.id = :usuarioId AND (p.fechaDevolucion IS NULL OR p.fechaDevolucion > :fechaActual)", Prestamo.class)
